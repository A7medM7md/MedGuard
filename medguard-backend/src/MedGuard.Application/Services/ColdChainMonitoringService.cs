...
public class ColdChainMonitoringService : IColdChainMonitoringService
{
    private readonly IUnitOfWork _uow;

    public ColdChainMonitoringService(IUnitOfWork uow) => _uow = uow;

    public async Task<SensorReadingDto> RecordReadingAsync(RecordReadingRequest request, CancellationToken ct = default)
    {
        ...
        return new SensorReadingDto(reading.Id, reading.BatchId, reading.DeviceId, reading.TemperatureC,
            reading.HumidityPercent, reading.Latitude, reading.Longitude, reading.RecordedAtUtc);
    }

    public async Task<IReadOnlyList<SensorReadingDto>> GetHistoryAsync(Guid batchId, int take = 100, CancellationToken ct = default)
    {
        var readings = await _uow.SensorReadings.GetByBatchIdAsync(batchId, take, ct);
        return readings.Select(r => new SensorReadingDto(
