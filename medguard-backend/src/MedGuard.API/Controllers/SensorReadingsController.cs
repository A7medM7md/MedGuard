...
public class SensorReadingsController : BaseApiController
{
    ...
    public SensorReadingsController(ResponseHandler response, IColdChainMonitoringService monitoringService) =>
    ...
    public async Task<ActionResult<Response<SensorReadingDto>>> Record(RecordReadingRequest request, CancellationToken ct) =>
        NewResult(await _monitoringService.RecordReadingAsync(request, ct));

    [HttpGet("batch/{batchId:guid}")]
    public async Task<ActionResult<Response<List<SensorReadingDto>>>> GetHistory(
        Guid batchId, [FromQuery] int take = 100, CancellationToken ct = default) =>
        NewResult(await _monitoringService.GetHistoryAsync(batchId, take, ct));
}
